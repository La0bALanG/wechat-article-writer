...
allowed-tools: tavily-mcp(tavily-search、tavily-extract)， amap-maps，Read, Write, Edit, Bash
triggers:
...
使用 `tavily-mcp.tavily-search` MCP 提供的搜索能力进行3-5轮搜索：
...
注意：调用 各个MCP 服务的工具叫做mcporter，安装目录：

```bash
~/.nvm/versions/node/v24.14.0/bin/mcporter
```

如果不知道有哪些可调用的MCP服务，可以使用如下命令查询：
...
```bash
mcporter call [服务名].[工具名] --args '{"参数1": "值1", "参数2": "值2"}'
```
...
使用自定义的 `tavily-mcp.tavily-extract` MCP 工具获取2-5篇代表性文章的完整内容：
...
```bash
cd ~/.openclaw/skills/wechat-tech-writer

conda activate llm_env

python scripts/generate_image_by_text.py \
...
### 步骤8：发送文件给用户（WeCom自建应用发送）

完成文章生成和图片生成并保存后，必须将这些生成的文件发送给用户。

执行 `scripts/wecom_send_file.py` 将生成的Markdown文章、封面图和任何其他配图一一发给用户。

**发送命令**：

```bash
# 发送文章MD文件
python scripts/wecom_send_file.py --file "{主题名称}.md"

# 发送封面图
python scripts/wecom_send_file.py --file cover.png

# 发送其他内容配图（如果有）
python scripts/wecom_send_file.py --file performance-comparison.png
```

如果用户指定了具体的接收人，可以使用 `--user` 参数，例如 `--user yutian-anweichao`，默认发送给所有人(`@all`)。

***

### 步骤9：智能排版并上传至微信公众号草稿箱

**⚠️ 这是最终步骤**：将生成的 Markdown 文件连同封面图，提交进行智能排版后，静默推送至公众号草稿箱中。

执行 `scripts/upload_to_wechat_draft.py` 来完成：

**上传命令**：

```bash
python scripts/upload_to_wechat_draft.py "{主题名称}.md" "cover.png"
```

注意：

- 该脚本会自动调用在线智能排版服务以及图片自动转换，确保公众号预览效果最佳。
- 必须传递明确的文章名和封面名字。
...
- [ ] 语言自然流畅，适合公众号风格
- [ ] 字数在2000-3000字之间
- [ ] ✅ **已将生成的所有文件（文章和图片）发送给用户**
- [ ] ✅ **已调用脚本完成智能排版和草稿箱上传（步骤9）**
...
- **[api-configuration.md](references/api-configuration.md)** - 生图API配置指南（即梦AI密钥设置）
...
- **scripts/generate\_image\_by\_text.py** - 即梦AI文生图API调用脚本（封面图、内容配图的主要工具）
- **scripts/generate\_image\_by\_image.py** - 即梦AI图生图API调用脚本（基于参考图生成新图片）
- **scripts/generate\_video\_by\_text\_or\_firstImage.py** - 即梦AI文/图生视频API调用脚本
- **scripts/wecom\_send\_file.py** - 企业微信发送文件脚本（用于将生成的结果发送给用户）
...
```bash
python scripts/generate_image_by_image.py \
  --image_url "参考图URL" \
  --prompt "编辑提示词" \
  --output output.png
```
...
```bash
# 文生视频
python scripts/generate_video_by_text_or_firstImage.py \
  --prompt "视频描述提示词" \
  --output output.mp4
...
# 步骤8：发送文件给用户
执行以下命令：
python scripts/wecom_send_file.py --file "Claude_Sonnet_4介绍.md"
python scripts/wecom_send_file.py --file "cover.png"
if 文章有性能对比数据:
    python scripts/wecom_send_file.py --file "performance-comparison.png"

# 步骤9：推送至微信草稿箱
python scripts/upload_to_wechat_draft.py "Claude_Sonnet_4介绍.md" "cover.png"
```
